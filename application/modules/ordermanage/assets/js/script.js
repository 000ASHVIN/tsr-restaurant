...
function setTotalCookTime(seconds, type) {
    var total = $("#cookedtime").val();
    var a = total.split(':'); // split it at the colons
    // minutes are worth 60 seconds. Hours are worth 60 minutes.
    var totalSeconds = parseInt((a[0]) * 60 * 60) + parseInt((a[1]) * 60) + parseInt(a[2]);
    totalSeconds = isNaN(totalSeconds) ? 0 : totalSeconds;
    totalSeconds = (type == 'add') ? (totalSeconds + seconds) : (totalSeconds - seconds);
    ...
    $("#cookedtime").val(date.toISOString().substr(11, 8));
    return;
}

function calculateCookingTime(type) {    
    const itemCookTime = $('#addfoodlist .cart_item_cooktime');    
    if (itemCookTime.length) {
        itemCookTime.each(function(index, fooditem) {
            const hms = $(fooditem).data('cook-time');
            var a = hms.split(':'); // split it at the colons
            // minutes are worth 60 seconds. Hours are worth 60 minutes.
            var seconds = (+a[0]) * 60 * 60 + (+a[1]) * 60 + (+a[2]);            
            setTotalCookTime(seconds, type);
        });
    } else {
        const path = window.location.pathname;
        if (path.includes("order/pos_invoice")) {
            setTotalCookTime(0, 'reset');
        }
    }
}
...
function checkProductionStockAvailability(element, pid, vid, extra = null) {    
    let cartRow = $(element).closest(".class_product_"+pid);
    if ($(".class_product_"+pid).length == 0) {        
        return true;
    }
    let totalStock = parseInt(cartRow.data('stock'));
    let singleStock = parseInt($(".class_product_"+pid).data(String(vid)));    
    if (!cartRow.length) {
        return true;
    }
    let qtyToAdd = (extra != null) ? extra : 1;
    let currentqty = parseInt($("#productionsetting-"+pid+"-"+vid).text()) + qtyToAdd;    
    let stockedUsed = 0;   
    $(".class_ing_"+cartRow.data('ing-id')).each(function(i, item) {        
        stockedUsed += parseInt($(item).find("span[id^='productionsetting-']").text())  * parseInt($(item).data('single-stock'));
    });
    stockedUsed += (singleStock * qtyToAdd);
...
function posaddonsfoodtocart(pid, id, more = null) {
    const element = $("span[id^=productionsetting-"+pid+"-]").first();
    const canAdd = checkProductionStockAvailability(element, pid, $("#sizeid_1").val(), parseInt($("#itemqty_1").val()));
    if (!canAdd) {
        alert('Please check Ingredients! \n Some Ingredients are not Available!');
        return false;
    }
