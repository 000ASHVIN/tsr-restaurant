...
<?php
$isAdminUser = $this->session->userdata('isAdmin');
    ...
    <div class="panel-body">
      <div class="tabsection"> <span class="display-none"><?php echo $settinginfo->language; ?></span>
        <ul class="nav nav-tabs mb-2" role="tablist">
          ...
          <li><a href="#kitchen" role="tab" data-toggle="tab" class="torder newtab 456" id="kitchenorder" onclick="giveselecttab(this)"><i class="fa fa-coffee smallview"></i> <span class="responsiveview"><?php echo display('kitchen_status'); ?></span> </a> </li>
          <?php if ($qrapp == 1) { ?>
          ...
          <?php } ?>
          <?php if ($isAdminUser) { ?>
            <li class="seelist"> <a href="#settings" role="tab" data-toggle="tab" class="comorder newtab 963" id="todayonlieorder" onclick="giveselecttab(this)"><i class="fa fa-shopping-bag smallview"></i> <span class="responsiveview"><?php echo display('onlineord'); ?></span> </a> <a href="" class="notif"><span class="label label-danger count">0</span></a> </li>
            <li> <a href="#messages" role="tab" data-toggle="tab" class="torder newtab 123" id="todayorder" onclick="giveselecttab(this)"><i class="fa fa-first-order smallview"></i> <span class="responsiveview"><?php echo display('tdayorder'); ?></span> </a> </li>
          <?php } ?>
                        ...
                        <div class="product-category">
                          ...
                          ?>
                            ...
                            <?php } else { ?>
                              ...
                              <div class="listcatnew cat-nav manualDropdown" onclick="getslcategory(<?php echo $category->CategoryID; ?>)"><?php echo $category->Name; ?></div>
                          <?php  }
                          } ?>
                          ...
                          <div class="product-grid">
                            <div class="row row-m-3" id="product_search">
                              ...
                              foreach ($itemlist as $item) {
                                ...
                                $query = $CI->db->query("SELECT * FROM production_details  WHERE `foodid` = '$productID' AND `pvarientid` = '$variantId' ");
                                $row = $query->row();

                                $ingredientid = $row->ingredientid;

                                $stock = $CI->db->query("SELECT * FROM ingredients  WHERE `id` = '$row->ingredientid'");
                              ...
                              ?>
                                <div class="col-xs-6 col-sm-4 col-md-4 col-lg-3 col-p-3">
                                  <div class="panel panel-bd product-panel select_product">
                                    ...
                                    <input type="hidden" name="select_product_stock" class="select_product_stock" value="<?php echo $stock->stock_qty; ?>">
                              ...
                              <table class="table table-bordered wpr_100" border="1" id="addinvoice">
                                ...
                                <tbody class="itemNumber">
                                  ...
                                  foreach ($cart as $item) {
                                    ...
                                    $_productionDetailOfVarient = $this->db->select('*')->from('production_details')->where('foodid', $iteminfo->ProductsID)->where('pvarientid', $item['sizeid'])->get()->row_array();
                                    $_productionDetailOfVarientAll = $this->db->select('*')->from('production_details')->where('foodid', $iteminfo->ProductsID)->get()->result_array();
                                    $qtyAsPerIng = [];
                                    $_ids = array_column($_productionDetailOfVarientAll, 'qty');
                                    $_pvarientids = array_column($_productionDetailOfVarientAll, 'pvarientid');
                                    $varQtys = array_combine($_pvarientids, $_ids);
                                    $dataAttr = '';
                                    foreach ($varQtys as $vId => $sTock) {
                                      $dataAttr .= 'data-' . $vId . '="' . $sTock . '" ';
                                    }
                                    $ingredients = $this->db->select('*')->from('ingredients')->where('id', $_productionDetailOfVarient['ingredientid'])->get()->row_array();
                                    ...
                                    $i++;
                                  ?>
                                    <tr id="<?php echo $i; ?>" <?= $dataAttr ?> data-ing-id="<?php echo $_productionDetailOfVarient['ingredientid']; ?>" class="cart_item_cooktime class_ing_<?php echo $_productionDetailOfVarient['ingredientid']; ?> class_product_<?php echo $iteminfo->ProductsID; ?>" data-cook-time="<?php echo $iteminfo->cookedtime; ?>" data-stock="<?php echo intval($ingredients['stock_qty']) ?>" data-single-stock="<?php echo $_productionDetailOfVarient['qty'] ?>">
                                      ...
                                      <td scope="row"><a class="btn btn-info btn-sm btnleftalign" onclick="posupdatecart(this,'<?php echo $item['rowid'] ?>',<?php echo $item['pid']; ?>,<?php echo $item['sizeid'] ?>,<?php echo $item['qty']; ?>,'add')"><i class="fa fa-plus" aria-hidden="true"></i></a> <span id="productionsetting-<?php echo $item['pid'] . '-' . $item['sizeid'] ?>"> <?php echo $item['qty']; ?> </span> <a class="btn btn-danger btn-sm btnrightalign" onclick="posupdatecart(this,'<?php echo $item['rowid'] ?>',<?php echo $item['pid']; ?>,<?php echo $item['sizeid'] ?>,<?php echo $item['qty']; ?>,'del')"><i class="fa fa-minus" aria-hidden="true"></i></a></td>
                                      <td width=""><?php if ($currency->position == 1) {
                                                      echo $currency->curr_icon;
                                                    } ?>
