...
#check stock
	public function checkingredientstock($foodid,$vid,$foodqty){
									...
									if($cart = $this->cart->contents()){
										$sumexitqty=0;
										  foreach ($cart as $item){
													 $checkanother=$this->db->select('*')->from('production_details')->where('foodid',$item['pid'])->where('pvarientid',$item['sizeid'])->where('ingredientid',$productiondetail->ingredientid)->get()->row();
													 $this->db->last_query();
														if(!empty($checkanother)){
															if($item['pid']==$foodid && $item['sizeid']==$vid){}
															else{
															 $sumexitqty+=$checkanother->qty*$item['qty'];
															}
														}
											
											}
											$newqty=$sumexitqty+$r_stock;
															$this->db->select('*');
															$this->db->from('ingredients');
															$this->db->where('id', $productiondetail->ingredientid);
															$this->db->where('stock_qty >=',$newqty);
															$stockcheck2 = $this->db->get()->num_rows();
															if($stockcheck2 == 0){
																	return 'Please check Ingredients!!Some Ingredients are not Available!!!';
															}
									}
		...
		if(!empty($productiondetails)){
		   ...
		   foreach($productiondetails as $productiondetail){
			/* Bug fix added on 20-04-2023 */
			$currentCartContents = $this->cart->contents();
			$cartQty = 0;
			if (!empty($currentCartContents)) {
				$cartItemsQty = array_column($currentCartContents, 'qty');
				$cartQty = array_sum($cartItemsQty);
			}
			// $availableStock = $cartQty < $this->getTotalStock($productiondetail->ingredientid);
			// /* Bug fix added on 20-04-2023 */
			$r_stock = $productiondetail->qty*$foodqty;
			// $r_stock = $productiondetail->qty*$foodqty + $cartQty;
			// $stockcheck = $r_stock <= $availableStock;
			// echo $availableStock."<-->".$r_stock."<-->".$cartQty;
			// var_dump($stockcheck);die;
			// var_dump($r_stock);die;
			/*add stock in ingredients*/
				$this->db->select('*');
				$this->db->from('ingredients');
				$this->db->where('id', $productiondetail->ingredientid);
...
public function checkingredient($nitqty,$ingredientid,$foodid,$proqty){
	...
	public function getTotalStock($ingredientid = 0) {
		$availableStock = 0;
		if ($ingredientid == 0) {
		...
		if (!is_bool($stockQuery)) {
			$stock = $stockQuery->row_array();
			$availableStock = $stock['stock_qty'];
		}
